...
function filterTiles(state: FilterStore, tiles: Tile[]) {
  return tiles.filter((tile) => {
    const matchCollection = matchAll(state.selectedCollections, [tile.collection?.id ?? -1])
    ...
    const matchOutdoorIndoor = matchAll(state.selectedOutdoorIndoor, [tile.outdoorIndoor?.id ?? -1])

    return matchCollection && matchSize && matchSurface && matchFeature && matchColor && matchOutdoorIndoor
  })
}
...
export const useFilterStore = create<FilterStore>((set, get) => ({
  getCount: (type, id, tiles) => {
    ...
    return tiles.filter((tile) => {
      ...
      const matchSize = ignore('size') ? true : matchAll(state.selectedSizes, tile.sizes?.map((s) => s.size.id) ?? [])
      ...
      const matchColor = ignore('color')
        ? true
        : matchAll(state.selectedColors, tile.colors?.map((c) => c.color.id) ?? [])
      ...
      switch (type) {
        ...
        case 'size':
          ...
          break
        case 'surface':
          matchesTypeId = tile.surfaces?.some((s) => s.surface.id === id) ?? false
          break
        case 'feature':
          matchesTypeId = tile.features?.some((f) => f.feature.id === id) ?? false
          break
        case 'color':
  ...
  recalcFilteredTiles: (tiles: Tile[]) => {
    ...
    let matches = tiles.filter((tile) => {
      const matchCollection =
        ...
        (tile.collection?.id !== undefined && state.selectedCollections.includes(tile.collection.id))
      const matchSize =
      ...
      const matchOutdoorIndoor =
        ...
        (tile.outdoorIndoor?.id !== undefined && state.selectedOutdoorIndoor.includes(tile.outdoorIndoor.id))
      return matchCollection && matchSize && matchSurface && matchFeature && matchColor && matchOutdoorIndoor
    })
    if (state.sortBy) {
