...
export const TileList = () => {
  ...
  const { filteredTiles, sortBy, setSortBy, recalcFilteredTiles, setFilteredTiles } = useFilterStore()
  const { loading, setTiles } = useTileStore()
  ...
  const currentTiles = filteredTiles.slice(startIndex, startIndex + tilesPerPage)

  useEffect(() => {
    setTiles(data || [])
    setFilteredTiles(data || [])
  }, [data, setTiles, setFilteredTiles])
  ...
  return (
    <div className='flex flex-wrap justify-center gap-6 px-2 w-full items-start self-start'>
      <div className='w-full flex flex-col gap-5 md:gap-10 px-6'>
        <h1 className={`text-left w-full text-3xl md:text-5xl ${figtree.className}`}>{t('title')}</h1>
        <div className='flex items-center justify-between w-full flex-col md:flex-row gap-4'>
          <div className='flex items-center justify-start text-[#888888]'>
            <h2 className='w-fit'>{tp('showing', { start, end, total: filteredTiles.length })}</h2>
          </div>

          <div className='relative w-full gap-2 flex flex-row-reverse justify-between'>
            <Select value={sortBy ?? undefined} onValueChange={(val) => onSelect(val as SortOption)}>
      ...
      <div className='w-full flex justify-center'>
        <Pagination currentPage={currentPage} totalPages={totalPages} handlePageChange={setCurrentPage} tp={tp} />
      </div>
    </div>
  )
}
